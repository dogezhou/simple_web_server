...
# -*- coding: utf-8 -*-
"""
把 webserver2 改写为 并发的服务器（使用 Python 的 threading 模块）
测试：
> python webserver5.py wsgiapp:app
"""
...
class WSGIServer(object):
    ...
    def serve_forever(self):
        ...
        while True:
            ...
            # 处理一个请求，然后关闭客户端连接。然后
            # 循环等待下一个客户端连接
            t = threading.Thread(target=self.process_thread, args=())  # self.handle_one_request()
            t.start()

    def process_thread(self):
        # 创建一个当前线程的请求上下文
        self.local_request = RequestSession(self.client_connection,
                                            self.application,
                                            self.server_name,
                                            self.server_port)
        # 处理线程中的请求
        self.local_request.handle_one_request()


class RequestSession(object):
    """
    为每个请求创建实例，保持请求内容, 处理请求
    """
    ...
    def get_environ(self):
        ...
        env['PATH_INFO'] = self.path  # /%E5%91%A8%E4%BC%9F 浏览器编码过的 url 需要 unquote解码
    ...
    def finish_response(self, result):
        ...
        try:
            ...
            print('type(client_connention)=', type(self.client_connection))
