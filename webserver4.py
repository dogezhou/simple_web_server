...
# -*- coding: utf-8 -*-
"""
！！！！ 错误，各线程可能共享了self.connetion 和其他变量，会出错, 用多进程没问题？
把 webserver2 改写为 并发的服务器（使用 Python 的 threading 模块）

测试：
> python webserver4.py wsgiapp:app
"""
import os
import socket
...
class WSGIServer(object):
    ...
    def serve_forever(self):
        ...
        while True:
            ...
            # 处理一个请求，然后关闭客户端连接。然后
            # 循环等待下一个客户端连接
            t = Process(target=self.handle_one_request, args=()) # self.handle_one_request()
            t.start()
    ...
    def finish_response(self, result):
        ...
        try:
            ...
            print('子进程：{id}结束'.format(id=os.getpid()))
